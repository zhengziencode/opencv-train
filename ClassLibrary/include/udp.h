		...
		if (flag == SEND)
		{
			if (!addressInitalized)
			{
				...
				do
				{
					...
					recvfrom(Sock, recvBuf, 256, 0, (sockaddr*)&clientAddr, &AddrLen);
...
bool udpFileTranform::sendFile(){
	...
	int count = (int)fileLen / 456;//sending loops
	int lastBytes = fileLen % 456;//the bytes of final package.
...
// 		sendto(Sock, buf, 466, 0, (sockaddr*)&serverAddr, sizeof(sockaddr_in));
		...
		int readLen = fread(buf+10, 1 ,456, fp);
		sendto(Sock, buf, 466, 0, (sockaddr*)&clientAddr, sizeof(sockaddr_in));
		if (status==START)
		{
			status = CONTINUE;
		}
...
// 	printf("fileLLen==%d\n", fileLLen);
	while (status!=END)
	{
		...
		readLen+=fwrite(buf+10, 1, 456, fp);
